-- Durable SIE imports create terminal operation receipts, which must be
-- removed during verified sandbox teardown while company_settings still exists.
-- Keep real-company retention and all terminal UPDATE enforcement unchanged.
...
BEGIN
  IF OLD.status IN ('succeeded', 'failed', 'cancelled') THEN
    -- Invoker context matters: SECURITY DEFINER here would make every caller
    -- appear to be the owner and turn a caller-controlled flag into a bypass.
